...
class Room {
    ...
    void printUnencryptedNameIfPossiblyRelevant() {
        char[] unencryptedChars = encryptedName.replaceAll("-", " ").toCharArray();
        char c, forwardRotationAmount = (char) (sectorID % 26);
        for (int i = 0; i < unencryptedChars.length; i++) {
            c = unencryptedChars[i];
            if (c == ' ') continue;
            c += forwardRotationAmount;
            if (c > 'z') c -= 26;
            unencryptedChars[i] = c;
        }
        String unencryptedString = String.valueOf(unencryptedChars);
        if (unencryptedString.contains("north") || unencryptedString.contains("pole")
                || unencryptedString.contains("object") || unencryptedString.contains("storage"))
            System.out.println("Unencrypted string (Sector ID "+sectorID+"): "+unencryptedString);
    }
